...
if(trim($_REQUEST['action_perform']) == "add_project")
{
    ...
    //exit;
	 $trans_type = 4;
    $trans_type_name = "sale_goods" ;
   
	$trans_id = mysql_real_escape_string(trim($_REQUEST['trans_id']));
	
	
	$query="insert into payment_plan set trans_id = '".$trans_id."', project_id = '".$project_id."', debit = '".$amount."', description = '".$description."', on_customer = '".$cust_id."', payment_date = '".strtotime($_REQUEST['payment_date'])."',subdivision = '".$subdivision."',trans_type = '".$trans_type."', trans_type_name = '".$trans_type_name."', create_date = '".getTime()."'";
    
	$result= mysql_query($query) or die('error in query '.mysql_error().$query);
	
	$link_id_1 = mysql_insert_id();
	
	$query2="insert into payment_plan set trans_id = '".$trans_id."', cust_id = '".$cust_id."', credit = '".$amount."', description = '".$description."', on_project = '".$project_id."', payment_date = '".strtotime($_REQUEST['payment_date'])."',link_id = '".$link_id_1."',subdivision = '".$subdivision."',trans_type = '".$trans_type."', trans_type_name = '".$trans_type_name."', create_date = '".getTime()."'";
...
<div id="wrapper">
	...
	<form name="project_form" id="project_form" action="" method="post" enctype="multipart/form-data" >
		<table width="95%">
			...
			<td>
			<!--<select name="subdivision" id="subdivision"  style="width:140px; height:20px;"  >
                     <option value="">-- Please Select --</option> 
					 <?php 
					 echo $subdivision_query = "select * from subdivision ORDER BY name ASC";
					 ...
					 {
					 	?>
						<option value="<?php echo $subdivision_data['id']; ?>"><?php echo $subdivision_data['name']; ?></option> 
						<?php
					 }
					 ?>
