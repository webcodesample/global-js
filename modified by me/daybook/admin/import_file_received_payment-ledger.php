...
if(trim($_REQUEST['trans_receivepayment']) == "receive_payment_yes")
{
	...
	$trans_type = 2;
    $trans_type_name = "receive_payment" ;
			...
			*/	 
			$insert_f = 0;
				...
				if($to_name_n[$ii]=="new")
				 {
					...
					$to_arr = explode("- ",$to_name_n[$ii]);
					$bank_id_n = get_field_value("id","bank","bank_account_name",$to_arr[0]);
				...
				if($insert_f!=1)
				{
					...
					$query="insert into payment_plan set trans_id = '".$trans_id."', bank_id = '".$bank_id."', credit = '".$amount."', description = '".$description."', on_customer = '".$cust_id."', on_project = '".$project_id."', payment_date = '".$payment_date."', trans_type = '".$trans_type."', trans_type_name = '".$trans_type_name."',file_import_id='".$file_id."', file_import_data_id='".$trans_flag_n[$ii]."',create_date = '".getTime()."'";
					$result= mysql_query($query) or die('error in query '.mysql_error().$query);
					
					$link_id_1 = mysql_insert_id();
					
					$query2="insert into payment_plan set trans_id = '".$trans_id."', cust_id = '".$cust_id."', debit = '".$amount."', description = '".$description."', on_project = '".$project_id."', on_bank = '".$bank_id."', payment_date = '".$payment_date."',link_id = '".$link_id_1."',trans_type = '".$trans_type."', trans_type_name = '".$trans_type_name."', file_import_id='".$file_id."', file_import_data_id='".$trans_flag_n[$ii]."',create_date = '".getTime()."'";
	 ...
	 //exit;
	 header("Location:import_file_received_payment-ledger.php?file_id=$file_id");
...
{
	$select_query = "select * from tbl_info where file_import_id = '".$_REQUEST['file_id']."' and final_payment_type='received_payment' and action_flag!=1 ORDER BY payment_date DESC";
	$select_result = mysql_query($select_query) or die('error in query select bank query '.mysql_error().$select_query);
	$select_total = mysql_num_rows($select_result);
...
<form name="search_form" id="search_form" action="" method="post" onSubmit="return search_valid();" >
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
				<td width="50">&nbsp;&nbsp;From</td>
					<td width="220"><input type="text"  name="from_date" id="from_date" value="<?php echo $_REQUEST['from_date']; ?>" style="width:150px;" autocomplete="off" />&nbsp;<img src="js/images2/cal.gif" onClick="javascript:NewCssCal('from_date')" style="cursor:pointer"/></td>
				    <td width="50">&nbsp;&nbsp;To</td>
					<td width="220"><input type="text"  name="to_date" id="to_date" value="<?php echo $_REQUEST['to_date']; ?>" style="width:150px;" autocomplete="off" />&nbsp;<img src="js/images2/cal.gif" onClick="javascript:NewCssCal('to_date')" style="cursor:pointer"/></td>
			
                      <td align="left" valign="top" >&nbsp;&nbsp;<input type="submit" name="search_button" id="search_button" value="Search" class="button"  />&nbsp;&nbsp;<input type="button" name="refresh" id="refresh" value="Refresh" class="button" onClick="window.location='import_file_received_payment-ledger.php?file_id=<?php echo $_REQUEST['file_id']; ?>';"  /></td>
...
<?php include_once("main_body_open.php") ?>
		...
		<table class="data" id="my_table" border="1" cellpadding="1" cellspacing="0" style="border: 1px solid #111111; width:100%;">
				...
				while($select_data = mysql_fetch_array($select_result))
				{
					...
					<tr class="data">
                       ...
                       <td class="data" align="center"><?php echo $select_data['file_series']; ?>	</td>
						...
						$sql 	= "select cust_id,full_name from `customer` where full_name like '%$from_val1%' and type = 'customer'";
					...
					<?php
						while($row = mysql_fetch_assoc($query)){
							...
							?>
							<option value="<?php echo $val; ?>" <?php if($select_data['from_name']==$row['full_name']){ echo "selected='selected'"; } ?> ><?php echo $row['full_name']; ?></option> 
					 ...
					 <option value="new">New</option>
						...
						$to_val1=substr($select_data['to_name'],0,4);
							$sql 	= "select bank_account_name,bank_account_number from `bank` where bank_account_name like '%".$to_val1."%'";
							$query 	= mysql_query($sql);  ?>
						 ...
						 <?php  while($row = mysql_fetch_assoc($query)){
								$val = $row['bank_account_name'].' - '.$row['bank_account_number']; ?>
		               			<option value="<?php echo $val; ?>" <?php if($select_data['to_name']==$row['bank_account_name']){ echo "selected='selected'"; } ?> ><?php echo $row['bank_account_name']; ?></option> 
       							<?php 	} 	//echo $select_data['to_name']; ?>
							<option value="new">New</option>
							</select>    
						</td>
						<td class="data">
			...
			else
			{
				?>
				<tr class="data" >
					<td  width="30px" colspan="14" class="record_not_found" >Record Not Found</td>
				</tr>
				<?php
			}
			?>
		...
		<input type="hidden" name="trans_receivepayment" id="trans_receivepayment" value="" >
...
{
	if(confirm("Are you sure to Import this data in Receive Payment file?!!!!!......"))
	{
		$("#trans_receivepayment").val("receive_payment_yes");
		$("#search_form1").submit();
		return true;
	}
}
