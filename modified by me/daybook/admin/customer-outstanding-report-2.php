...
if(mysql_real_escape_string(trim($_REQUEST['search_val1'])) == "project_s_start")
{
   ...
   $select_query = "select *,payment_plan.id as payment_id  from payment_plan inner join customer on payment_plan.cust_id = customer.cust_id  and payment_plan.cust_id!='' and payment_plan.cust_id > 0 and payment_plan.on_project='".$pay_bank_id."' and payment_plan.on_project > 0 and customer.type='customer'  group by payment_plan.cust_id  ORDER BY customer.full_name ASC ";
...
}else if(mysql_real_escape_string(trim($_REQUEST['search_val1'])) == "customer_s_start")
{
  ...
  //  $select_query = "select * from customer where cust_id=".$pay_bank_id." and type = 'customer' ORDER BY full_name ASC LIMIT $startResults, $resultsPerPage";
    $select_query = "select *,payment_plan.id as payment_id  from payment_plan inner join customer on payment_plan.cust_id = customer.cust_id  and payment_plan.cust_id ='".$pay_bank_id."' and payment_plan.cust_id > 0 and payment_plan.on_project!='' and payment_plan.on_project > 0 and customer.type='customer'  group by payment_plan.cust_id  ORDER BY customer.full_name ASC ";   
   }
            ...
            if($select_total > 0)
            {
                ...
                while($select_data = mysql_fetch_array($select_result))
                {
                    ...
                    }else
                    {
                     ...
                     $select_query_pro = "select *,id as payment_id ,max(payment_date) as maxpayment_date, SUM(debit) as total_debit,SUM(credit) as total_credit from payment_plan  where cust_id = '".$select_data['cust_id']."' ".$pro_val."  and  on_project!='' and on_project > 0   group by on_project   ";
  // echo $select_query;
    ...
    $select_total_pro = mysql_num_rows($select_result_pro);
                 ...
                 while($select_data_pro = mysql_fetch_array($select_result_pro))
                {   
                    $ii=$i+$startResults;    
                    ...
                    <tr class="data">
                        ...
                        <td class="data">&nbsp;<a href="customer-ledger.php?cust_id=<?php echo $select_data_pro['cust_id']; ?>" title="View Ledger"  ><?php 
                        //echo $select_data['fname'];
                        echo get_field_value("fname","customer","cust_id",$select_data_pro['cust_id']);
                         ?></a></td>
                        
                        <td class="data">&nbsp;<?php //echo $select_data['mobile'];
                        echo get_field_value("name","project","id",$select_data_pro['on_project']);
                        
                         ?></td>
                        <td class="data" align="center">&nbsp;<?php //echo date("d-m-Y",$select_data_pro['maxpayment_date']);   ?>
                        <?php 
                        $select_query_pay1 = "select max(payment_date) as payment_max_date from payment_plan  where cust_id = '".$select_data_pro['cust_id']."'  and  on_project ='".$select_data_pro['on_project']."' and trans_type_name in('receive_goods','sale_goods','internal_transfer','inst_receive_goods','inst_sale_goods','instmulti_sale_goods')   ";
                    ...
                    echo date("d-m-Y",$select_data_pay1['payment_max_date']);
                        ...
                        <?php 
                        $select_query_pay = "select max(payment_date) as payment_max_date from payment_plan  where cust_id = '".$select_data_pro['cust_id']."'  and  on_project ='".$select_data_pro['on_project']."' and trans_type_name in('make_payment','receive_payment','loanadvance_makepayment','loanadvance_receivepayment','inst_make_payment','inst_receive_payment','instmulti_receive_payment')   ";
                       // echo  $select_query_pay;
