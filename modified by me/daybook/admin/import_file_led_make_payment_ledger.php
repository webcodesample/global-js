...
if(trim($_REQUEST['trans_makepayment']) == "make_payment_yes")
{
	...
	$payment_method_n=$_REQUEST['payment_method_n'];
	$check_no_old=$_REQUEST['check_no_old'];

	//$file_attach  //payment_method_n,check_no_old
    $trans_flag_no = mysql_real_escape_string(trim($_REQUEST['trans_flag_no'])); 
	$trans_type = 5;
    $trans_type_name = "loanadvance_makepayment" ;
				...
				if($to_name_n[$ii]=="new")
				 {
					...
					$to_arr = $to_name_n[$ii];
					//$cust_id_n = $to_arr[1];
					$client_id_n = get_field_value("id","loan_advance","name",$to_arr);
				}
			/*	if($project_n[$ii]=="new")
				 {
					//$from_name_old[$ii];
...
*/
				...
				/*	while($wi<1)
					{
						...
						$qry_max=" select max(advance_loan_no) as max_loan_id from payment_plan";
					...
					$pay_method = $payment_method_n[$ii];
					$pay_checkno = $check_no_old[$ii];
					//$trans_flag_n[$ii]; ,file_id ,pay_checkno,pay_method  //payment_method_n,check_no_old
					//payment_plan_link1 ,payment_plan_link2 ,action_flag
					//file_import_id='".file_id."', file_import_data_id='".trans_flag_n[$ii]."'
					$query="insert into payment_plan set advance_loan_no = '".$loan_id_new."',on_loan = '".$client_id."', bank_id = '".$bank_id."', debit = '".$amount."', description = '".$description."',  payment_date = '".$payment_date."', payment_method = '".$pay_method."',payment_checkno = '".$pay_checkno."', trans_type = '".$trans_type."', trans_type_name = '".$trans_type_name."',file_import_id='".$file_id."', file_import_data_id='".$trans_flag_n[$ii]."',create_date = '".getTime()."'";
					...
					$query2="insert into payment_plan set advance_loan_no = '".$loan_id_new."',loan_id = '".$client_id."', on_bank = '".$bank_id."',  credit = '".$amount."', description = '".$description."', payment_date = '".$payment_date."',payment_method = '".$pay_method."',payment_checkno = '".$pay_checkno."', link_id = '".$link_id_1."',trans_type = '".$trans_type."', trans_type_name = '".$trans_type_name."',file_import_id='".$file_id."', file_import_data_id='".$trans_flag_n[$ii]."', create_date = '".getTime()."'";
	 ...
	 header("Location:import_file_led_make_payment_ledger.php?file_id=$file_id");
...
{
	$select_query = "select * from tbl_info where file_import_id = '".$_REQUEST['file_id']."' and final_payment_type='lad_make_payment' and action_flag!=1 ORDER BY payment_date DESC";
	$select_result = mysql_query($select_query) or die('error in query select bank query '.mysql_error().$select_query);
	$select_total = mysql_num_rows($select_result);
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				<td width="50">&nbsp;&nbsp;From</td>
					...
					<td width="120"><input type="text"  name="to_date" id="to_date" value="<?php echo $_REQUEST['to_date']; ?>" style="width:100px;" autocomplete="off" />&nbsp;<img src="js/images2/cal.gif" onClick="javascript:NewCssCal('to_date')" style="cursor:pointer"/></td>
                      ...
                      <td align="left" valign="top" >&nbsp;&nbsp;<input type="submit" name="search_button" id="search_button" value="Search" class="button"  />&nbsp;&nbsp;<input type="button" name="refresh" id="refresh" value="Refresh" class="button" onClick="window.location='import_file_led_make_payment_ledger.php?file_id=<?php echo $_REQUEST['file_id']; ?>';"  /></td>
...
<?php include_once("main_body_open.php") ?>
		...
		<table class="data" id="my_table" border="1" cellpadding="1" cellspacing="0" style="border: 1px solid #111111; width:100%;">
            ...
            <thead class="report-header">
				...
				<th class="data"><b>Pay-Method</b></th>
				<th class="data"><b>Check No.</b></th>
				...
				while($select_data = mysql_fetch_array($select_result))
				{
					...
					<tr class="data">
                       ...
                       <td class="data" align="center"><?php echo $select_data['file_series']; ?>	</td>
						...
						$sql 	= "select name from `loan_advance` where name like '%$to_val1%'";
						$query 	= mysql_query($sql);
						?>
					...
					<?php
						while($row = mysql_fetch_assoc($query)){
							...
							$val = $row['name'];  
							?>
							<option value="<?php echo $val; ?>" <?php if($select_data['to_name']==$row['name']){ echo "selected='selected'"; } ?> ><?php echo $row['name']; ?></option> 
							<?php 	} 	//echo $select_data['from_name']; ?>
					 <option value="new">New</option>
					 </select>    
						...
						<td class="data">
						<select id="<?php echo "payment_method_n$inm"; ?>" name='payment_method_n[]' style="width:100px; height: 25px;">
						...
						<td class="data">
						<input style="width: 150px;"  type='text' value="" id="<?php echo "check_no_old$inm"; ?>" name='check_no_old[]'/>
