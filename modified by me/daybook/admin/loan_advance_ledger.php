...
//,payment_plan.id as payment_id
   $select_query = "select *,payment_plan.id as payment_id  from payment_plan inner join loan_advance on payment_plan.loan_id = loan_advance.id  and payment_plan.loan_id!='' and payment_plan.loan_id > 0  and payment_plan.loan_id='".$_REQUEST['loan_id']."' ".$from_datedata." ".$to_datedata."    ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
    ...
    $select_query3_bal = "select SUM(debit) as total_debit,SUM(credit) as total_credit from payment_plan inner join loan_advance on payment_plan.loan_id = loan_advance.id  and payment_plan.loan_id!='' and payment_plan.loan_id > 0  and payment_plan.loan_id='".$_REQUEST['loan_id']."' ".$from_datedata." ".$to_datedata."      ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
     ...
     $select_query3 = "select SUM(debit) as total_debit,SUM(credit) as total_credit from payment_plan inner join loan_advance on payment_plan.loan_id = loan_advance.id  and payment_plan.loan_id!='' and payment_plan.loan_id > 0  and payment_plan.loan_id='".$_REQUEST['loan_id']."' ".$from_datedata." ".$to_datedata."     ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
                ...
                $select_query3_1 = "select SUM(debit) as total_debit,SUM(credit) as total_credit from payment_plan inner join loan_advance on payment_plan.loan_id = loan_advance.id  and payment_plan.loan_id!='' and payment_plan.loan_id > 0  and payment_plan.loan_id='".$_REQUEST['loan_id']."' ".$to_datedata."      ";
                ...
                $select_query5 = "select SUM(debit) as total_debit,SUM(credit) as total_credit from payment_plan inner join loan_advance on payment_plan.loan_id = loan_advance.id  and payment_plan.loan_id!='' and payment_plan.loan_id > 0  and payment_plan.loan_id='".$_REQUEST['loan_id']."' ".$from_datedata." ".$to_datedata."  group by payment_plan.loan_id     ";
}
else
{
     $select_query = "select *,payment_plan.id as payment_id  from payment_plan inner join loan_advance on payment_plan.loan_id = loan_advance.id  and payment_plan.loan_id!='' and payment_plan.loan_id > 0  and payment_plan.loan_id='".$_REQUEST['loan_id']."'    ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
    // echo $select_query;
    ...
    $select_query2 = "select * ,payment_plan.id as payment_id from payment_plan inner join loan_advance on payment_plan.loan_id = loan_advance.id  and payment_plan.loan_id!='' and payment_plan.loan_id > 0  and payment_plan.loan_id='".$_REQUEST['loan_id']."'      ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
    $select_result2 = mysql_query($select_query2) or die('error in query select customer query '.mysql_error().$select_query2);
    $select_total2 = mysql_num_rows($select_result2);
    
    $select_query3_bal = "select SUM(debit) as total_debit,SUM(credit) as total_credit from payment_plan inner join loan_advance on payment_plan.loan_id = loan_advance.id  and payment_plan.loan_id!='' and payment_plan.loan_id > 0  and payment_plan.loan_id='".$_REQUEST['loan_id']."'      ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
                $select_result3_bal = mysql_query($select_query3_bal) or die('error in query select cash query '.mysql_error().$select_query3_bal);
                $select_data3_bal = mysql_fetch_array($select_result3_bal);
                $bal=$select_data3_bal['total_credit']-$select_data3_bal['total_debit'];
    //echo $bal;
...
<body  data-home-page-title="" class="u-body u-xl-mode" data-lang="en">
  ...
  <?php include_once("main_heading_open.php") ?>
    ...
    <tr>
        ...
        Loan & Advances - <?php echo get_field_value("name","loan_advance","id",$_REQUEST['loan_id']); ?> Ledger</h4>
  </td>
        <td width="" style="float:right;">
        <a href="loan_advance.php" title="Back" ><input type="button" name="back_button" id="back_button" value="" class="button_back"  /></a><input type="button" name="print_button" id="print_button" value="" class="button_print" onClick="return print_data();"  />
...
</table><input type="hidden" id="print_header" name="print_header" value="<h3>Loan & Advances - <?php echo get_field_value("name","loan_advance","id",$_REQUEST['loan_id']); ?> Ledger</h3>">   
  ...
  <form name="search_form" id="search_form" action="" method="post" onSubmit="return search_valid();" >
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <input type="text"  name="from_date" id="from_date" value="<?php echo $_REQUEST['from_date']; ?>" style="width:120px; height: 25px;" autocomplete="off" />&nbsp;<img src="js/images2/cal.gif" onClick="javascript:NewCssCal('from_date')" style="cursor:pointer"/>
                 </td>
                 ...
                 <td width="50">
                    ...
                    <input type="text"  name="to_date" id="to_date" value="<?php echo $_REQUEST['to_date']; ?>" style="width:120px; height: 25px;" autocomplete="off" />&nbsp;<img src="js/images2/cal.gif" onClick="javascript:NewCssCal('to_date')" style="cursor:pointer"/>
                 </td>
                 
                    <td align="left" valign="top" >&nbsp;&nbsp;<input type="submit" name="search_button" id="search_button" value="Search" class="button"  />&nbsp;&nbsp;<input type="button" name="refresh" id="refresh" value="Refresh" class="button" onClick="window.location='loan_advance_ledger.php?loan_id=<?php echo $_REQUEST['loan_id']; ?>';"  /></td>
  ...
  <?php include_once("main_body_open.php") ?>
       ...
       <div id="ledger_data">
        ...
        <tr style="display:none ;">
            <td><b>Loan & Advances Ledger  :</b></td><td><b><?php echo get_field_value("name","loan_advance","id",$_REQUEST['loan_id']); ?></b></td>
        ...
        <td><b>Period Start :</b></td><td><b><?php if($_REQUEST['from_date']!=""){ echo date("d-m-Y",strtotime($_REQUEST['from_date']));  }?></b></td>
            ...
            <thead class="report-header">
                ...
                <th class="data noExl" id="header1"><b>File</b></th>
                </thead>
            </tr>
            <?php
            if($select_total > 0)
            {
              ...
              //echo $bal;
                ...
                while($select_data = mysql_fetch_array($select_result))
                {
                    if($i > 1)
                    {
                        $select_query4 = "select debit,credit from payment_plan where loan_id = '".$_REQUEST['loan_id']."' and id = '".$temp_payment_id."' LIMIT 0,1  ";
...
?>
                    ...
                    <tr class="data">
                       ...
                       <?php   if($select_data['advance_loan_no'] != "" && $select_data['advance_loan_no'] != 0) 
                        { ?>
                        &nbsp;<a href="javascript:account_transaction(<?php echo $select_data['advance_loan_no'] ?>);"><img src="mos-css/img/delete.png" title="Delete" ></a>
                        <?php } ?>
                        ...
                        ?>
                         <?php   if($select_data['advance_loan_no'] != "" && $select_data['advance_loan_no'] != 0) 
                        { ?>
                        &nbsp;<a href="javascript:void(0);" title="Attach File" onClick="return attach_file_function('<?php echo $select_data['payment_id']; ?>');" ><img src="images/images.jpg" width="20" ></a>   <?php } ?>
                        <?php
                        ...
                        {  ?>
                        <a href="edit-make-loan-advance.php?id=<?php echo $select_data['payment_id']; ?>&ledger=loan_advance"><img src="mos-css/img/edit.png" title="Edit"></a>
                 <?php  } ?>
                        <?php
                        if($select_data['trans_type_name']=="loanadvance_receivepayment")
                        {  ?>
                         <a href="edit-receive-loan-advance.php?id=<?php echo $select_data['payment_id']; ?>&ledger=loan_advance"><img src="mos-css/img/edit.png" title="Edit"></a>
                 <?php  } ?>
