...
if(trim($_REQUEST['action_perform']) == "add_customer_payment")
{
	/*echo '<pre>';
	print_r($_REQUEST);
	exit;*/
	...
	$cust_id = $from_arr[1];
	$bank_id = get_field_value("id","bank","bank_account_number",$to_arr[1]);
	$amount=mysql_real_escape_string(trim($_REQUEST['amount']));
	$description=mysql_real_escape_string(trim($_REQUEST['description']));
	
	
	$query="insert into payment_plan set bank_id = '".$bank_id."', credit = '".$amount."', description = '".$description."', create_date = '".getTime()."'";
	$result= mysql_query($query) or die('error in query '.mysql_error().$query);
	
	$query2="insert into payment_plan set cust_id = '".$cust_id."', debit = '".$amount."', description = '".$description."', create_date = '".getTime()."'";
	$result2= mysql_query($query2) or die('error in query '.mysql_error().$query2);
	
	
	$msg = "customer_payment added successfully.";
	
}

/*     Deletion  Account   */

if($_REQUEST['action_perform'] == "delete_customer_payment")
{
	...
	$msg = "customer_payment Deleted Successfully.";
...
{
	$from_arr = explode("- ",$_REQUEST['search_text']);
	$cust_id = $from_arr[1];
...
<div id="wrapper">
	...
	<?php } ?>
	<a href="javascript:void(0);" onClick="return add_div();" > <h2>Recive</h2> </a>
	
	<div id="adddiv" style="display:<?php if($error_msg != "") { ?>block<?php } else { ?>none<?php } ?>;">
	
	<form name="customer_payment_form" id="customer_payment_form" action="" method="post" >
		<table width="95%">
		...
		<form name="search_form" id="search_form" action="" method="post" onSubmit="return search_valid();" >
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td align="left" valign="top" >&nbsp;&nbsp;<input type="submit" name="search_button" id="search_button" value="Search" class="button"  />&nbsp;&nbsp;<input type="button" name="refresh" id="refresh" value="Refresh" class="button" onClick="window.location='customer-payment.php';"  /></td>
...
function validation()
{
	...
	{
		$("#action_perform").val("add_customer_payment");
		$("#customer_payment_form").submit();
		return true;
	}
...
function account_delete(del_id)
{
	...
	{
		$("#action_perform").val("delete_customer_payment");
		$("#del_id").val(del_id);
		$("#customer_payment_form").submit();
		return true;
	}
