...
if(mysql_real_escape_string(trim($_REQUEST['bank_search'])) != "")
{
    ...
    $query = "select * from bank where id ='".$pay_bank_id."' and type = 'cash account' ORDER BY bank_account_name ASC";
    //echo $query;
    //exit;
...
{
    $column = $_REQUEST['search_type'];
    $query = "select * from bank where $column LIKE '%".mysql_real_escape_string(trim($_REQUEST['search_text']))."%' and type = 'cash account' ORDER BY bank_account_name ASC";
    $result = mysql_query($query) or die('error in query select bank query '.mysql_error().$query);
    $total_row = mysql_num_rows($result);
}
...
{
    $query = "select * from bank where type = 'cash account' ORDER BY bank_account_name ASC";
    $result = mysql_query($query) or die('error in query select bank query '.mysql_error().$query);
    $total_row = mysql_num_rows($result);
}
$page = $_REQUEST['page'];
...
if(mysql_real_escape_string(trim($_REQUEST['bank_search'])) != "")
{
   ...
   if($pay_from_arr[0] =="All"){
       $select_query = "select * from bank where  type = 'cash account' ORDER BY bank_account_name ASC ";
   }else{
    $select_query = "select * from bank where id ='".$pay_bank_id."'  and type = 'cash account' ORDER BY bank_account_name ASC LIMIT $startResults, $resultsPerPage";   
   }
...
{
    $column = $_REQUEST['search_type'];
    $select_query = "select * from bank where $column LIKE '%".mysql_real_escape_string(trim($_REQUEST['search_text']))."%' and type = 'cash account' ORDER BY bank_account_name ASC LIMIT $startResults, $resultsPerPage";
    $select_result = mysql_query($select_query) or die('error in query select bank query '.mysql_error().$select_query);
    $select_total = mysql_num_rows($select_result);
}
else
{
    $select_query = "select * from bank where type = 'cash account' ORDER BY bank_account_name ASC LIMIT $startResults, $resultsPerPage";
    $select_result = mysql_query($select_query) or die('error in query select bank query '.mysql_error().$select_query);
    $select_total = mysql_num_rows($select_result);
}
...
<table style="width:100%;" border="0" style="padding:0px 0px; margin-top:-10px;">
    <tr>
        ...
        Cash Account List     </h4>
  </td>
        <td width="" style="float:right; margin-right:0px;">
        <a href="add_cash.php"  ><input type="button" name="add_button" id="add_button" value="" class="button_add" /></a>
