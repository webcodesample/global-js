<section class="u-align-left u-clearfix u-grey-5 u-section-1" id="carousel_9bc8">
    
    <div class="u-clearfix u-sheet u-sheet-1" style="padding:0px; ">
    
<nav id="main-nav " style="margin-top:-20px;">
      <!-- Sample menu definition -->
      <ul id="main-menu" class="sm sm-blue">
      <li><a></a>
    <ul>
    <li><a href="#">Money</a>
    <ul>
        
                <li><a href="bank.php">Bank</a></li>
                <li><a href="internal-transfer.php">Internal Transfer</a></li>
                <li><a href="make-payment.php">Make Payment</a></li>
                <li><a href="receive-payment.php">Receive Payment</a></li>
                <li><a href="cash.php">Cash</a></li>
</ul>
</li>
                <li><a href="">invoice</a>
                    <ul>
                      <li ><a href="">Sale Invoice </a>
                          <ul>
                              <li><a href="instant-sale-invoice_multiple.php">Create Sale Invoice</a></li>
                              <li><a href="invoice-list.php">Invoice Listing</a></li>
                          </ul>
                      </li>
                      <li ><a href=""> Purchase Invoice</a>
                          <ul>
                              <li><a href="instant-receive-goods.php">Create Purchase Invoice</a></li>
                              <li><a href="purchase_good_list.php">Purchase Invoice Listing</a></li>
                          </ul>
                      </li>
                    </ul>
                </li>
                
              <li ><a href="">Projects & Subdivisions</a>
                  <ul>
                      <li><a href="project.php">Projects</a></li>
                      <li><a href="subdivision.php">Subdivisions</a></li>
                      <li><a href="#"></a></li>
                  </ul>
              </li>
              <li ><a href="">Customers & Suppliers</a>
                  <ul>
                      <li><a href="customer.php">Customers</a></li>
                      <li><a href="supplier.php">Suppliers</a></li>
                      <li><a href="#"></a></li>
                  </ul>
              </li>
              
              <li ><a href="">Loan & Advances</a>
                  <ul>
                      <li><a href="loan_advance.php">L&A Ledger</a></li>
                      <li><a href="make-loan-advance.php">Make Payment</a></li>
                      <li><a href="receive-loan-advance.php">Receive Payment</a></li>
                  </ul>
              </li>
              <li ><a href="">GST & TDS</a>
                <ul>
                  <li><a href="#">GST</a>
                  <ul>
                      <li><a href="input_gst_subdivision.php">GST Subdivision</a></li>
                      <li><a href="output_gst_subdivision.php">GST Ledger</a></li>
                      <li><a href="gst-list.php">GST List</a></li>
                      <li><a href="hsn-list.php">HSN Code LIST</a></li>
                  </ul>
                  </li>
                  <li><a href="#">TDS</a>
                  <ul>
                      <li><a href="input_tds.php">Create TDS Subdivision</a></li>
                      <li><a href="output_tds.php">TDS By Customer</a></li>
                      <li><a href="tds-list.php">TDS Listing</a></li>
                  </ul>
                  </li>
              
                </ul>
                  
              </li>
              
              <li ><a href="">Document</a>
                  <ul>
                      <li><a href="category.php">Add Category</a></li>
                      <li><a href="document.php">Document List</a></li>
                  </ul>
              </li>
              
              <li ><a href="">Reports</a>
                  <ul>
                     <li><a href="invoice-report.php">Sales Reports</a></li>
                      <li><a href="purchase-report-new.php">Purchase Reports</a></li>
                      <!--<li><a href="gst-report.php">GST Reports</a></li>-->
                      <li><a href="testgstr.php">GST Return</a></li>
                  </ul>
              </li>
              
              <li ><a href="">Admin</a>
                  <ul>
                      <li><a href="user.php">User</a></li>
                      <li><a href="invoice-issuer.php">Invoice Issure</a></li>
                      <li><a href="import_file.php">Import File</a></li>

                  </ul>
              </li>
                         
    </ul>
    </li>      
    <li><a href="#">Money</a>
    <ul>
        
                <li><a href="bank.php">Bank</a></li>
                <li><a href="internal-transfer.php">Internal Transfer</a></li>
                <li><a href="make-payment.php">Make Payment</a></li>
                <li><a href="receive-payment.php">Receive Payment</a></li>
                <li><a href="cash.php">Cash</a></li>
</ul>
</li>
                <li><a href="">invoice</a>
                    <ul>
                      <li ><a href="">Sale Invoice </a>
                          <ul>
                              <li><a href="instant-sale-invoice_multiple.php">Create Sale Invoice</a></li>
                              <li><a href="invoice-list.php">Invoice Listing</a></li>
                          </ul>
                      </li>
                      <li ><a href=""> Purchase Invoice</a>
                          <ul>
                              <li><a href="instant-receive-goods.php">Create Purchase Invoice</a></li>
                              <li><a href="purchase_good_list.php">Purchase Invoice Listing</a></li>
                          </ul>
                      </li>
                    </ul>
                </li>
                
              <li ><a href="">Projects & Subdivisions</a>
                  <ul>
                      <li><a href="project.php">Projects</a></li>
                      <li><a href="subdivision.php">Subdivisions</a></li>
                      <li><a href="#"></a></li>
                  </ul>
              </li>
              <li ><a href="">Customers & Suppliers</a>
                  <ul>
                      <li><a href="customer.php">Customers</a></li>
                      <li><a href="supplier.php">Suppliers</a></li>
                      <li><a href="#"></a></li>
                  </ul>
              </li>
              
              <li ><a href="">Loan & Advances</a>
                  <ul>
                      <li><a href="loan_advance.php">L&A Ledger</a></li>
                      <li><a href="make-loan-advance.php">Make Payment</a></li>
                      <li><a href="receive-loan-advance.php">Receive Payment</a></li>
                  </ul>
              </li>
              <li ><a href="">GST & TDS</a>
                <ul>
                  <li><a href="#">GST</a>
                  <ul>
                      <li><a href="input_gst_subdivision.php">GST Subdivision</a></li>
                      <li><a href="output_gst_subdivision.php">GST Ledger</a></li>
                      <li><a href="gst-list.php">GST List</a></li>
                      <li><a href="hsn-list.php">HSN Code LIST</a></li>
                  </ul>
                  </li>
                  <li><a href="#">TDS</a>
                  <ul>
                      <li><a href="input_tds.php">Create TDS Subdivision</a></li>
                      <li><a href="output_tds.php">TDS By Customer</a></li>
                      <li><a href="tds-list.php">TDS Listing</a></li>
                  </ul>
                  </li>
              
                </ul>
                  
              </li>
              
              <li ><a href="">Document</a>
                  <ul>
                      <li><a href="category.php">Add Category</a></li>
                      <li><a href="document.php">Document List</a></li>
                  </ul>
              </li>
              
              <li ><a href="">Reports</a>
                  <ul>
                     <li><a href="invoice-report.php">Sales Reports</a></li>
                      <li><a href="purchase-report-new.php">Purchase Reports</a></li>
                      <!--<li><a href="gst-report.php">GST Reports</a></li>-->
                      <li><a href="testgstr.php">GST Return</a></li>
                  </ul>
              </li>
              
              <li ><a href="">Admin</a>
                  <ul>
                      <li><a href="user.php">User</a></li>
                      <li><a href="invoice-issuer.php">Invoice Issure</a></li>
                      <li><a href="import_file.php">Import File</a></li>

                  </ul>
              </li>        <!--    <li><a href="#">Mega menu</a>
                <ul class="mega-menu">
                    <li>
                           <div style="width:400px;max-width:100%;">
                <div style="padding:5px 24px;">
                  <p>This is a mega drop down test. Just set the "mega-menu" class to the parent UL element to inform the SmartMenus script. It can contain <strong>any HTML</strong>.</p>
                  <p>Just style the contents as you like (you may need to reset some SmartMenus inherited styles - e.g. for lists, links, etc.)</p>
                </div>
              </div>
            </li>
          </ul>
        </li>-->
      </ul>
    </nav>

    <div class="u-clearfix u-expanded-width u-gutter-0 u-layout-wrap u-layout-wrap-1" style="padding:0px;">
      
    <!-- jQuery -->
    <script type="text/javascript" src="menu_libs/jquery/jquery.js"></script>

<!-- SmartMenus jQuery plugin -->
<script type="text/javascript" src="jquery.smartmenus.js"></script>

<!-- SmartMenus jQuery init -->
<script type="text/javascript">
  $(function() {
    $('#main-menu').smartmenus({
      subMenusSubOffsetX: 1,
      subMenusSubOffsetY: -8
    });
  });
</script>

