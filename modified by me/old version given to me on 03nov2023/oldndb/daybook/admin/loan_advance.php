...
{
	//client_type,other_name,customer_name,supplier_name,mobile,
//email,current_address,opening_balance,opening_balance_date,company_name,another_contact_name,another_mobile
	
    
    //name,type,type_id,mobile,email,current_address,company_name,opening_balance,opening_balance_date,another_contact_name,another_mobile,create_date,update_date
...
if(mysql_real_escape_string(trim($_REQUEST['search_action'])) != "")
{
    ...
    if($to_date!=""){
     ...
     if($search_val!="All"){
        //LIKE '%".mysql_real_escape_string(trim($_REQUEST['loan_client']))."%'
    ...
    //and payment_plan.loan_advance='".$_REQUEST['loan_advance']."'
   // $query = "select * from loan_advance ".$loan_advancedata."  ORDER BY name ASC";
...
if(mysql_real_escape_string(trim($_REQUEST['search_action'])) != "")
{
    ...
    if($to_date!=""){
     ...
     if($search_val!="All"){
        $loan_advancedata =" and loan_advance.name LIKE '%".mysql_real_escape_string(trim($_REQUEST['loan_client']))."%'";
       //  $loan_advancedata =" and loan_advance.id = '".mysql_real_escape_string(trim($_REQUEST['loan_client']))."'";
     }else { $loan_advancedata=""; }
...
else
{
   ...
   //  $select_query = "select *,payment_plan.id as payment_id , SUM(payment_plan.debit) as total_debit,SUM(payment_plan.credit) as total_credit ,count(payment_plan.id) as no_entry from payment_plan inner join loan_advance on payment_plan.loan_advance = loan_advance.id  and payment_plan.loan_advance!='' and payment_plan.loan_advance > 0 group by payment_plan.loan_advance ORDER BY loan_advance.name ASC LIMIT $startResults, $resultsPerPage";
	$select_result = mysql_query($select_query) or die('error in query select loan_advance query '.mysql_error().$select_query);
	$select_total = mysql_num_rows($select_result);
}
