...
if(mysql_real_escape_string(trim($_REQUEST['search_action'])) != "")
{
	...
	if($cust_id == "ALL")
	{
		...
		$query = "select * from payment_plan inner join customer on payment_plan.cust_id = customer.cust_id  and payment_plan.cust_id != '' and payment_plan.payment_date >= '".$from_date."' and payment_plan.payment_date <= '".$to_date."' and customer.type = 'supplier' GROUP BY payment_plan.cust_id ORDER BY payment_plan.payment_date ASC";
	}
	else
	{
		$query = "select * from payment_plan inner join customer on payment_plan.cust_id = customer.cust_id and payment_plan.cust_id = '".$cust_id."' and payment_plan.payment_date >= '".$from_date."' and payment_plan.payment_date <= '".$to_date."' and customer.type = 'supplier' GROUP BY payment_plan.cust_id ORDER BY payment_plan.payment_date ASC";
	}
...
<div id="wrapper">
    ...
    <input type="hidden" id="print_header" name="print_header" value="Supplier - Reports">
	<?php if($msg != "") { ?>
	<div class="sukses">
		<?php echo $msg; ?>
		</div>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
					<td width="140" align="left" valign="middle"><select name="cust_id" id="cust_id" size="1" style="width:140px; height:20px;"  >
                        ...
                        <option value="ALL">---- ALL ----</option> 
                         <?php
				 	$sql_bank=mysql_query("select * from customer where type = 'supplier' ORDER BY full_name ASC ");
				...
				<?php	}?> 
				 ...
				 <td width="50">
					...
					<td align="left" valign="top" >&nbsp;&nbsp;<input type="submit" name="search_button" id="search_button" value="Search" class="button"  />&nbsp;&nbsp;<input type="button" name="refresh" id="refresh" value="Refresh" class="button" onClick="window.location='supplier-report.php';"  /></td>
					...
					<tr class="data">
						...
						if($cust_id == "ALL")
						{
							...
							while($date_list2 = mysql_fetch_array($date_list_result2))
							{
									...
									$date_list_query3 = "select * from payment_plan inner join customer on payment_plan.cust_id = customer.cust_id  and payment_plan.cust_id = '".$select_data['cust_id']."' and payment_plan.payment_date = '".$date_list2['payment_date']."' and customer.type = 'supplier' ";
