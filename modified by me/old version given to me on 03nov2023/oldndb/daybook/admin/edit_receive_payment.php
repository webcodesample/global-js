...
if(trim($_REQUEST['action_perform']) == "add_payment")
{
    ...
    $query="update payment_plan set bank_id = '".$bank_id."', credit = '".$amount."', description = '".$description."', on_customer = '".$cust_id."', on_project = '".$project_id."', payment_date = '".$payment_date."',update_date = '".getTime()."' where  id = '".$id_second1."'";
    $result= mysql_query($query) or die('error in query '.mysql_error().$query);
    
    
    
    $query2="update  payment_plan set  cust_id = '".$cust_id."', debit = '".$amount."', description = '".$description."', on_project = '".$project_id."', on_bank = '".$bank_id."', payment_date = '".$payment_date."', update_date = '".getTime()."' where  id = '".$id_first1."'";
...
<body  data-home-page-title="" class="u-body u-xl-mode" data-lang="en">
  ...
  <?php include_once("main_heading_open.php") ?>
    ...
    <?php } ?>    
        ...
        <?php 
        if($_REQUEST['trsns_pname']=="customer-ledger")
    { ?><a href="customer-ledger.php?cust_id=<?php echo $old_cust_id; ?>" title="Back" style=""><input type="button" name="back_button" id="back_button" value="" class="button_back"  /></a>
...
<?php include_once("main_heading_close.php") ?>
  <?php include_once("main_body_open.php") ?>
    ...
    <input type="hidden" id="trsns_pname" name="trsns_pname" value="<?php echo $trsns_pname; ?>">
		...
		<table width="95%" style="padding:0px 20px 30px 30px;">
        <tr>
                <td><?php
    if($_REQUEST['trsns_pname']=="customer-ledger")
    { ?> 
    <h4 class="u-text-2 u-text-palette-1-base1 ">Update Receive Payment ( <?php  echo get_field_value("bank_account_name","bank","id",$old_bank_id);?>
<font size="3">&nbsp;(<?php echo date("d-m-Y",$select_data['payment_date']); ?>)</font>)</h4>
   
    <?php    
    }else if($_REQUEST['trsns_pname']=="bank-ledger")
    { ?> 
    <h4 class="u-text-2 u-text-palette-1-base1 ">Update Receive Payment ( <?php echo get_field_value("full_name","customer","cust_id",$old_cust_id); ?>
<font size="3">&nbsp;(<?php echo date("d-m-Y",$select_data['payment_date']); ?>)</font>)</h4>
...
?>
			...
			<tr><td >To</td>
            <?php
             $sql_bank     = "select bank_account_name,bank_account_number from `bank` where id=".$old_bank_id." ";
             $query_bank     = mysql_query($sql_bank);
             $select_bank = mysql_fetch_array($query_bank);
             
  
            ?>
			<td><input type="text" id="to"  name="to" value="<?php echo $select_bank['bank_account_name'].' - '.$select_bank['bank_account_number']; ?>" style="width:250px;"/></td></tr>
