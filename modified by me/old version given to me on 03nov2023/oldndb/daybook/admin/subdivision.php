...
if(mysql_real_escape_string(trim($_REQUEST['search_action'])) != "")
{
 ...
 if($search_val!="All"){
        //$subdivisiondata =" where subdivision.name LIKE '%".mysql_real_escape_string(trim($_REQUEST['subdivision']))."%'";
        // $subdivisiondata =" and subdivision.name LIKE '%".mysql_real_escape_string(trim($_REQUEST['subdivision']))."%'";
    ...
    //and payment_plan.subdivision='".$_REQUEST['subdivision']."'
   // $query = "select * from subdivision ".$subdivisiondata."  ORDER BY name ASC";
...
if(mysql_real_escape_string(trim($_REQUEST['search_action'])) != "")
{
   ...
   // echo $select_query;
   // $select_query = "select * from subdivision ".$subdivisiondata." ORDER BY name ASC LIMIT $startResults, $resultsPerPage";
...
else
{
   ...
   //  $select_query = "select *,payment_plan.id as payment_id , SUM(payment_plan.debit) as total_debit,SUM(payment_plan.credit) as total_credit ,count(payment_plan.id) as no_entry from payment_plan inner join subdivision on payment_plan.subdivision = subdivision.id  and payment_plan.subdivision!='' and payment_plan.subdivision > 0 group by payment_plan.subdivision ORDER BY subdivision.name ASC LIMIT $startResults, $resultsPerPage";
	$select_result = mysql_query($select_query) or die('error in query select subdivision query '.mysql_error().$select_query);
	$select_total = mysql_num_rows($select_result);
}
        ...
        <link rel="stylesheet" href="css/jquery-ui.css" />
          ...
          <div id="adddiv" style="display:<?php if($error_msg != "") { ?>block<?php } else { ?>none<?php } ?>;">
              ...
              <table width="" border="1" align="left" cellpadding="0" cellspacing="0">
                      ...
                      <tr>
                          <td valign="top">Subdivision Name</td>
                          <td valign="top">
                            <input type="text" id="subdivision2"  name="subdivision2" value="" style="width:250px;"/></td>
                                              </td>
                          <td></td>
                          <td ><br>&nbsp;&nbsp;<input type="submit" name="search_button" id="search_button" value="Search" class="button"  />&nbsp;&nbsp;<input type="button" name="refresh" id="refresh" value="Refresh" class="button" onClick="window.location='subdivision.php';"  />
                          </td>
                      </tr>
