...
//Internal Transfer
//internal_transfer
if(trim($_REQUEST['trans_internal_transfer']) == "internal_transfer_yes")
{
	...
	/*			 echo $trans_flag_n[$ii];
			...
			*/	 
			$insert_f = 0;
				 if($from_name_n[$ii]=="new")
				 {
					...
					$from_bank_id_n = get_field_value("id","bank","bank_account_name",$from_arr[0]);
				}
				if($to_name_n[$ii]=="new")
				 {
					$insert_f=1;
					...
					$to_arr = explode("- ",$to_name_n[$ii]);
					$to_bank_id_n = get_field_value("id","bank","bank_account_name",$to_arr[0]);
				...
				if($insert_f!=1)
				{
					...
					$trans_id=$trans_id_n;
					$to_bank_id=$to_bank_id_n;
					$amount = $amount_n[$ii];
					$description = $description_n[$ii];
					$from_bank_id = $from_bank_id_n;
					...
					$query="insert into payment_plan set trans_id = '".$trans_id."', bank_id = '".$from_bank_id."', debit = '".$amount."', description = '".$description."', on_bank = '".$to_bank_id."', payment_date = '".$payment_date."',trans_type = '".$trans_type."', trans_type_name = '".$trans_type_name."',file_import_id='".$file_id."', file_import_data_id='".$trans_flag_n[$ii]."',create_date = '".getTime()."'";
					...
					$query2="insert into payment_plan set trans_id = '".$trans_id."', bank_id = '".$to_bank_id."', credit = '".$amount."', description = '".$description."', on_bank = '".$from_bank_id."', payment_date = '".$payment_date."',link_id = '".$link_id_1."',trans_type = '".$trans_type."', trans_type_name = '".$trans_type_name."',file_import_id='".$file_id."', file_import_data_id='".$trans_flag_n[$ii]."', create_date = '".getTime()."'";
	 ...
	 //exit;
	 header("Location:import_file_internal_transfer-ledger.php?file_id=$file_id");
...
{
	$select_query = "select * from tbl_info where file_import_id = '".$_REQUEST['file_id']."' and final_payment_type='internal_payment' and action_flag!=1 ORDER BY payment_date DESC";
	$select_result = mysql_query($select_query) or die('error in query select bank query '.mysql_error().$select_query);
	$select_total = mysql_num_rows($select_result);
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				<td width="50">&nbsp;&nbsp;From</td>
					...
					<td width="120"><input type="text"  name="to_date" id="to_date" value="<?php echo $_REQUEST['to_date']; ?>" style="width:100px;" autocomplete="off" />&nbsp;<img src="js/images2/cal.gif" onClick="javascript:NewCssCal('to_date')" style="cursor:pointer"/></td>
					  ...
					  &nbsp;&nbsp;<input type="button" name="refresh" id="refresh" value="Refresh" class="button" onClick="window.location='import_file_internal_transfer-ledger.php?file_id=<?php echo $_REQUEST['file_id']; ?>';"  /></td>
...
<?php include_once("main_body_open.php") ?>
		...
		<input type="hidden" name="trans_internal_transfer" id="trans_internal_transfer" value="" >
...
function validation_import()
{
	...
	{
		$("#trans_internal_transfer").val("internal_transfer_yes");
		$("#search_form1").submit();
		return true;
	}
}
