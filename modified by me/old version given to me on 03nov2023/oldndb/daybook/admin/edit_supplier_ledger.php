...
if(trim($_REQUEST['action_perform']) == "add_project")
{
	...
	$msg = "supplier Ledger Update successfully.";
	$flag = 1;
...
$select_query = "select * from payment_plan where id=".$_REQUEST['id']." and trans_id=".$_REQUEST['trans_id']." and cust_id = '".$_REQUEST['cust_id']."'";
...
<div id="wrapper">
    ...
    <a href="supplier-ledger.php?cust_id=<?php echo $_REQUEST['cust_id']; ?>" title="Back" ><input type="button" name="back_button" id="back_button" value="" class="button_back"  /></a></div>
	<h3>Supplier - <?php echo get_field_value("full_name","customer","cust_id",$_REQUEST['cust_id']); ?> Ledger 
</h3>
    <h2>Update Receive Goods ( <?php  echo get_field_value("name","project","id",$select_data['on_project']);?>
<font size="3">&nbsp;(<?php echo date("d-m-Y",$select_data['payment_date']); ?>)</font>)</h2>
  ...
  <script src="js/jquery-ui.js"></script>
    ...
    <input type="hidden" id="id_cust" name="id_cust" value="<?php echo $select_data['id']; ?>">
    <input type="hidden" id="id_pro" name="id_pro" value="<?php echo $select_data_pro['id']; ?>">
		<table width="95%" >
...
?>
    ...
    $sql_cus     = "select cust_id,full_name from `customer` where cust_id=".$select_data['cust_id']." and type = 'supplier'";
...
?>
			...
			<tr><td >Project</td>
            <?php
                $project_nm = get_field_value("name","project","id",$select_data['on_project']);
...
<!--
			...
			<td><input type="text"  name="amount" id="amount" value="<?php echo $select_data['debit']; ?>" />&nbsp;<span style="color:#FF0000; font-weight:bold;"  >*</span></td></tr>
