...
if(mysql_real_escape_string(trim($_REQUEST['search_action'])) != "")
{
    ...
    { $to_datedata=""; 
    //$to_date=
   // $to_datedata2="and payment_plan.payment_date <= '".strtotime(toDate())."'";
    }
    if($_REQUEST['allentry']=="YES")
    {
        $payment_project="";
        $projectr_project="";
        //echo "hello<br>";
    ...
    {
        $payment_project=" and payment_plan.project_id = '".$_REQUEST['project_id']."'";
        $projectr_project="and project.id = '".$_REQUEST['project_id']."'";
    }
        $select_query = "select *,payment_plan.id as payment_id from payment_plan inner join project on payment_plan.project_id = project.id ".$payment_project." and payment_plan.subdivision = '".$_REQUEST['subdivision']."' ".$from_datedata." ".$to_datedata." ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
...
/*    $select_result = mysql_query($select_query) or die('error in query select cash query '.mysql_error().$select_query);
    $select_total = mysql_num_rows($select_result);
    */
    $select_query2 = "select *,payment_plan.id as payment_id from payment_plan inner join project on payment_plan.project_id = project.id ".$payment_project." and payment_plan.subdivision = '".$_REQUEST['subdivision']."' ".$from_datedata." ".$to_datedata." ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
    ...
    $select_total2 = mysql_num_rows($select_result2);
     ...
     $select_query3 = "select SUM(payment_plan.debit) as total_debit,SUM(payment_plan.credit) as total_credit from payment_plan inner join project on payment_plan.project_id = project.id ".$payment_project." and payment_plan.subdivision = '".$_REQUEST['subdivision']."' ".$from_datedata." ".$to_datedata." ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
                ...
                $select_query3_1 = "select SUM(payment_plan.debit) as total_debit,SUM(payment_plan.credit) as total_credit from payment_plan inner join project on payment_plan.project_id = project.id ".$payment_project." and payment_plan.subdivision = '".$_REQUEST['subdivision']."'  ".$to_datedata." ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
                ...
                $select_query5 = "select SUM(payment_plan.debit) as total_debit,SUM(payment_plan.credit) as total_credit from payment_plan inner join project on payment_plan.project_id = project.id ".$payment_project." and payment_plan.subdivision = '".$_REQUEST['subdivision']."' ".$from_datedata." ".$to_datedata." ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
...
else
{
    ...
    $select_query = "select *,payment_plan.id as payment_id from payment_plan inner join project on payment_plan.project_id = project.id ".$payment_project." and payment_plan.subdivision = '".$_REQUEST['subdivision']."' ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
    $select_result = mysql_query($select_query) or die('error in query select customer query '.mysql_error().$select_query);
    $select_total = mysql_num_rows($select_result);
   // echo  $select_query;
    $select_query2 = "select *,payment_plan.id as payment_id from payment_plan inner join project on payment_plan.project_id = project.id and project.id = '".$_REQUEST['project_id']."' and payment_plan.project_id = '".$_REQUEST['project_id']."' and payment_plan.subdivision = '".$_REQUEST['subdivision']."' ORDER BY payment_plan.payment_date DESC,payment_plan.id DESC ";
    $select_result2 = mysql_query($select_query2) or die('error in query select customer query '.mysql_error().$select_query2);
    $select_total2 = mysql_num_rows($select_result2);
 
        
    $select_query3 = "select SUM(debit) as total_debit,SUM(credit) as total_credit from payment_plan inner join project on payment_plan.project_id = project.id ".$projectr_project." ".$payment_project." and payment_plan.subdivision = '".$_REQUEST['subdivision']."'";
                $select_result3 = mysql_query($select_query3) or die('error in query select cash query '.mysql_error().$select_query3);
                $select_data3 = mysql_fetch_array($select_result3);
                //echo $select_data3['total_credit']-$select_data3['total_debit'];
                $bal=$select_data3['total_credit']-$select_data3['total_debit'];

                $select_query5 = "select SUM(debit) as total_debit,SUM(credit) as total_credit from payment_plan inner join project on payment_plan.project_id = project.id  ".$projectr_project." ".$payment_project."  and payment_plan.subdivision = '".$_REQUEST['subdivision']."'";
}
...
<body  data-home-page-title="" class="u-body u-xl-mode" data-lang="en">
  ...
  <?php include_once("main_heading_open.php") ?>
    ...
    <tr>
        ...
        Subdivision - <?php echo get_field_value("name","subdivision","id",$_REQUEST['subdivision']); ?></h4>
  </td>
        <td width="" style="float:right;">
            
            <a href="subdivision_projectlist.php?subdivision=<?php echo $_REQUEST['subdivision'];?>&from_date=<?php echo$_REQUEST['from_date']; ?>&to_date=<?php echo $_REQUEST['to_date']; ?>" title="Back" ><input type="button" name="back_button" id="back_button" value="Back" class="button_back"  /></a>
            <input type="button" name="print_button" id="print_button" value="Print" class="button_print" onClick="return print_data();"  />
                    &nbsp;
                   </script>
<script src="dist/jquery.table2excel.min.js"></script>
<!--<script src="jquery.min.js"></script>-->
<!--<script type="text/javascript" src="script.js"></script>-->
<input type="button" id="export_to_excel" value="Export" class="button_export" >
...
<?php include_once("main_search_open.php") ?>
  ...
  <form name="search_form" id="search_form" action="" method="post" onSubmit="return search_valid();" >
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td width="220">
                     <input type="text"  name="from_date" id="from_date" value="<?php echo $_REQUEST['from_date']; ?>" style="width:200px; height: 25px;" autocomplete="off" />&nbsp;<img src="js/images2/cal.gif" onClick="javascript:NewCssCal('from_date')" style="cursor:pointer"/>
                 ...
                 <td width="50">
                    ...
                    <td width="220">
                     <input type="text"  name="to_date" id="to_date" value="<?php echo $_REQUEST['to_date']; ?>" style="width:200px; height: 25px;" autocomplete="off" />&nbsp;<img src="js/images2/cal.gif" onClick="javascript:NewCssCal('to_date')" style="cursor:pointer"/>
                <!--    <input type="text" name="to_date" id="to_date" value="<?php echo $_REQUEST['to_date']; ?>"  readonly="" style="width:100px;" >-->
                 </td>
                 
                    <td align="left" valign="top" >&nbsp;&nbsp;<input type="submit" name="search_button" id="search_button" value="Search" class="button"  />&nbsp;&nbsp;<input type="button" name="refresh" id="refresh" value="Refresh" class="button" onClick="window.location='subdivision_ledger.php?subdivision=<?php echo $_REQUEST['subdivision']; ?>&project_id=<?php echo $_REQUEST['project_id']; ?>&allentry=<?php echo $_REQUEST['allentry']; ?> ';"  /></td>
            ...
            <input type="hidden" name="allentry" id="allentry" value="<?php echo $_REQUEST['allentry']; ?>"  />
            <input type="hidden" name="project_id" id="project_id" value="<?php echo $_REQUEST['project_id']; ?>"  />
 ...
 <!-------------->
  ...
  <?php include_once("main_body_open.php") ?>
      <input type="hidden" id="print_header" name="print_header" value=" <h3>Subdivision - <?php echo get_field_value("name","subdivision","id",$_REQUEST['subdivision']); ?> </h3><b>Project - <?php echo get_field_value("name","project","id",$_REQUEST['project_id']); ?> Ledger</b>">  
  ...
  <tr><td>
       ...
       Project - <?php echo get_field_value("name","project","id",$_REQUEST['project_id']); ?> Ledger</h4>
        ...
        <tr style="display:none ;">
            <td><b>Subdivision Name :</b></td><td><b> <?php echo get_field_value("name","subdivision","id",$_REQUEST['subdivision']); ?></b></td>
            <td><b> Project Name :</b></td><td><b><?php echo get_field_value("name","project","id",$_REQUEST['project_id']); ?></b></td>
...
?>
                    ...
                    <tr class="data">
                        ...
                        <td class="data"><?php echo date("d-m-y",$select_data['payment_date']); ?></td>
                         <td class="data"><?php 
                        if($select_data['project_id'] != "")
                        {
                            echo get_field_value("name","project","id",$select_data['project_id']);
                        }
                    ...
                    <?php  if($select_data['trans_type_name']=="instmulti_sale_goods")
                        ...
                        {  ?>
                        <a href="edit_project_ledger.php?project_id=<?php echo $select_data['project_id']; ?>&trans_id=<?php echo $select_data['trans_id']; ?>&id=<?php echo $select_data['payment_id']; ?>&back=subdivision"><img src="mos-css/img/edit.png" title="Edit"></a>
                 <?php  } ?>
                        <?php
                        if($select_data['trans_type_name']=="sale_goods")
                        {  ?>
                        <a href="edit_sale_invoice.php?trans_id=<?php echo $select_data['trans_id']; ?>&id=<?php echo $select_data['payment_id']; ?>&trsns_pname=<?php echo "project-ledger"; ?>&back=subdivision"><img src="mos-css/img/edit.png" title="Edit"></a>
                 <?php  } ?>
                         <?php
                        if($select_data['trans_type_name']=="inst_receive_goods")
                        {  ?>
                        <a href="edit-instant-receive-goods.php?trans_id=<?php echo $select_data['trans_id']; ?>&id=<?php echo $select_data['payment_id']; ?>&trsns_pname=<?php echo "project-ledger-inst-receive-goods"; ?>&back=subdivision"><img src="mos-css/img/edit.png" title="Edit"></a>
                 <?php  } ?>
                        
                        <?php
                        if($select_data['trans_type_name']=="inst_sale_goods")
                        {  ?>
                        <a href="edit-instant-sale-invoice.php?trans_id=<?php echo $select_data['trans_id']; ?>&id=<?php echo $select_data['payment_id']; ?>&trsns_pname=<?php echo "project-ledger-inst-sale-goods"; ?>&back=subdivision"><img src="mos-css/img/edit.png" title="Edit"></a>
                 <?php  } ?>
                 
                 <?php
                        if($select_data['trans_type_name']=="instmulti_sale_goods")
                        {  ?>
                        <a href="edit_instant-sale-invoice_multiple.php?trans_id=<?php echo $select_data['trans_id']; ?>&id=<?php echo $select_data['payment_id']; ?>&trsns_pname=<?php echo "project-ledger-inst-sale-goods"; ?>&back=subdivision"><img src="mos-css/img/edit.png" title="Edit"></a>
                 <?php  } ?>
                  <?php //////////////   edit File end     ///////////////?>
                           <?php //////////////   view File start     ///////////////?>                 
                  <?php
...
?><?php //////////////   view File end     ///////////////?>                  
                ...
                <?php
                $end_date1 = $select_data['payment_date'];
                    $i++;
                }
                 if($search_start=="1")
                {
                    ...
                    <tr class="data">
                        ...
                        if($from_date!=""){
                            ...
                            echo date("d-m-Y",$end_date1);
                        }
                         ?>
                        
                        <?php //echo date("d-m-Y",$from_date); ?></b></td>
