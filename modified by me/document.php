...
if(trim($_REQUEST['action_perform']) == "add_document")
{
	...
	else
	{
		...
		{
			$query="insert into document set category = '".$document_type."', name = '".$new_file_name."' , added_by = '".$_SESSION['userId']."', added_on = '".time()."'";
			...
			if($select_total > 0)
			{
				...
				while($select_data = mysql_fetch_array($select_result))
				{
					...
					<tr class="data">
						...
						<td class="data" >
						<input type="checkbox" name="email[]" id="email_<?php echo $i; ?>" value="document/<?php echo $select_data['name']; ?>" >
						&nbsp;
						<a href="document/<?php echo $select_data['name']; ?>" target="_blank" title="View File"><?php echo $select_data['name']; ?></a>
						</td>
